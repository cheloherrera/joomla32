...
class plgSystemAdmintoolsPro extends JPlugin
{
	...
	private function SQLiShield()
	{
		...
		foreach ($hashes as $hash)
		{
			...
			$allVars = $input->getArray(array(), null);
	...
	private function CSRFShield_ADVANCED()
	{
		...
		foreach ($hashes as $hash)
		{
			$allVars = JFactory::getApplication()->input->getArray(array(), null);
...
ENDINFO;
	...
	private function UploadShield()
	{
		...
		$filesHash = $input->getArray(array(), null);
    ...
    private function checkCustomAdminFolder()
    {
		...
		{
			JLoader::import('joomla.utilities.date');
			$jValid = new JDate($storedData->valid_to);
			if ($jValid->toUnix() < time())
			{
				$isValid = false;
			}
		}
