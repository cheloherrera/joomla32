<?php
/**
 * @version   $Id: default.php 6809 2013-01-27 22:04:15Z btowles $
	...
	<div class="rokstock-list">
		<?php if (count($tickers)): ?>
			...
			for ($i = 0; $i < count($tickers); $i++): ?>
				...
				if (preg_match("/\+/", $tickers[$i]["c"])) $direction = "plus"; else $direction = "minus";
...
//				$title = str_replace("&", "&amp;", $title);
				...
				<div class="row">
					<div class="title">
						...
						<a rel="<?php echo $tickers[$i]["e"] . ':' . $tickers[$i]["t"]; ?>" class="external" href="http://www.google.com/finance?q=<?php echo $tickers[$i]["e"] . ':' . $tickers[$i]["t"]; ?>"><span class="name"><?php echo str_replace('&', '&amp;', $title); ?></span></a>
		...
		<div class="rokstock-add">
			...
			<?php if ($show_main_chart): ?>
				<select class="rokstock-comparison">
					<?php
					for ($i = 0; $i < 4; $i++) {
						if ($i == 3) $sel = " selected='selected'"; else $sel = '';
