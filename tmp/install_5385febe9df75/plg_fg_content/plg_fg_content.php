...
class plgFeedgatorContent extends JPlugin
{
	...
	{
		if(!$this->params OR $force) {
			$this->params = new JParameter( $this->model->getParams($feedId), $this->model->getFilePath() );
		}
		return $this->params;
	}
	...
	*/
	...
	function getContentItemsQuery($where)
	{
		if(J_VERSION < 1.6) {
			...
			$query = '(SELECT	c.id AS id,
								c.title AS title,
								c.'.$this->state.' AS state,
								c.created AS created,
								c.ordering AS ordering,
								c.sectionid AS sectionid,
								c.catid AS catid,
								c.publish_up AS publish_up,
								c.publish_down AS publish_down,
								c.created_by_alias AS created_by_alias,
								c.created_by AS created_by,
								c.access AS access,
								c.checked_out AS checked_out,
								g.name AS groupname,
								cc.title AS cat_name,
								u.name AS editor,
								f.content_id AS frontpage,'.
								$this->section.' AS section_name,
								v.name AS author,
								fi.feed_id AS feedid,
								fg.title AS feed_title,
			...
			require_once(dirname(__FILE__).DS.'contentmodel.php');
	...
	function addDefaultImage($position,&$content,&$fgParams,$img = null)
	{
		...
		//to allow an over-ride
			...
			if($fgParams->get('feed_img')) {
				FeedgatorUtility::profiling('Adding Default Image');

				$base = $fgParams->get('rel_src',0) ? '/' : str_replace('administrator/','',JURI::base());
				$src = 'media/feedgator/images/feeds/'.$fgParams->get('feed_img');
				$savepath = JPATH_ROOT.'/'.$src;
				$img = '<img src="'.$base.$src.'" class="fgFeedImg" alt="'.$fgParams->get('title').'" />';
	...
	*/
	...
	function save(&$content,&$fgParams)
	{
		...
		if(J_VERSION >= 1.6) {
			...
			$row->language = $fgParams->get('feed_language') ? $fgParams->get('feed_language') : '*';
		}
		...
		$cache = JFactory::getCache($this->extensions);
		$cache->clean();
