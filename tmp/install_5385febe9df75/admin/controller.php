...
class FeedgatorController extends JController
{
	...
	function editFeed($default = false)
	{
		if($default) JRequest::setVar('cid',-2);
		$model = FGFactory::getFeedModel();
		$view = $this->getView('Feedgator','html');
		$view->setModel($model);
	...
	function saveFeed($apply = false, $default = false)
	{
		JRequest::checkToken() or die( 'Invalid Token' );
	...
	*/
	...
	function import($type=null)
	{
		...
		switch($type)
		{
			...
			if(JRequest::getCmd('task','') == 'fgautomator') {
	...
	function savePluginSettings($feedId = null, $content_type = null)
	{
		if($feedId === null) $feedId = JRequest::getInt('feedId',-2);
		if($content_type === null) $content_type = JRequest::getCmd('ext','');
	...
	function changePluginState()
	{
		...
		if($plugin->componentCheck()) {
			...
			$enabled		= (J_VERSION < 1.6) ? 'published' : 'enabled';

			$pquery = "SELECT $enabled FROM #__".$table_type."s WHERE element='$name' AND folder='feedgator'";
			$this->_db->setQuery($pquery);
			$row->published		= ($this->_db->loadResult() ? 0 : 1);

			if ($row->store()) {
				$query = "UPDATE #__".$table_type."s SET $enabled=$row->published WHERE element='$name' AND folder='feedgator'";
