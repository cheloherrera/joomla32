<?php
/* Adjusted for use in FeedGator by Matt Faulds. See "//FG edit" Last edit FG2.4 */
/**
...
class Readability
{
	...
	public $articleImages; //FG edit added to hold images
	public $articleVideos; //FG edit added to hold images
	public $dom;
	public $url = null; // optional - URL where HTML was retrieved
	public $debug = false; //FG edit
	public $debugMsg = ''; //FG edit
	public $fgParams = null; //FG edit
	protected $body = null; //
	...
	**/
	public $regexps = array(
        'likelyImageCandidates' => '/photos|images|gallery|x/i',
        'likelyVideoCandidates' => '/video|movie|clip|x/i',
	...
	const FLAG_GRAB_LIKELY_IMAGES = 1; //get likely images
	const FLAG_GRAB_LIKELY_VIDEOS = 1; //get likely videos
	...
	*/
	function __construct($html,$url=null,$fgParams)
	{
	...
	*/
	...
	/**
	* PreClean restricts content analysed based on parameters
	...
	*/
	protected function preClean() {
		...
		$parts = $this->fgParams->get('remove_by_attrib');
		$parts = explode(',',$parts);
		foreach($parts as $part) {
			if(strpos($part,'+') !== false) {
				...
				if(!empty($matches)) { // this is seriously ugly!
					$focus[$matches[1]][$matches[2]] = empty($focus[$matches[1]][$matches[2]]) ? array($matches[3]) : array_merge($focus[$matches[1]][$matches[2]],array($matches[3]));
				}
			} else {
				preg_match($regex,$part,$matches);
				if(!empty($matches)) { // this is seriously ugly!
					$clean[$matches[1]][$matches[2]] = empty($clean[$matches[1]][$matches[2]]) ? array($matches[3]) : array_merge($clean[$matches[1]][$matches[2]],array($matches[3]));
				}
			}

		}
		if(!empty($focus) OR !empty($clean)) {
			$body = $this->dom->getElementsByTagName('body')->item(0);
			$root = $body->parentNode;
			if(!empty($focus)) {
				$nBody = $this->dom->createElement('body');
				foreach($focus as $tag => $attribs) {
					$tl = $body->getElementsByTagName($tag);
					for($i=0;$i<$tl->length;$i++) {
						foreach($attribs as $k => $v) {
							if($tl->item($i)->getAttribute($k) AND in_array($tl->item($i)->getAttribute($k),$v)) {
								$tl->item($i)->getAttribute($k) ? $this->dbg('Focusing Specifically: kept ' . $tag . ' (' . $k . ':' . $tl->item($i)->getAttribute($k) . ')') :  $this->dbg('Focusing Specifically: kept ' . $tag . ' number '. $i) ;
			...
			if(!empty($clean)) {
				foreach($clean as $tag => $attribs) {
					if(isset($nBody)) {
						$tl = $nBody->getElementsByTagName($tag);
						for ($y=$tl->length-1; $y >= 0; $y--) {
							foreach($attribs as $k => $v) {
								if(in_array($tl->item($y)->getAttribute($k),$v)) {
									$this->dbg('Cleaning Specifically: removed ' . $tag . ' (' . $k . ':' . $tl->item($y)->getAttribute($k) . ')');
									$tl->item($y)->parentNode->removeChild($tl->item($y));
									break;
								}
							}
						}
	...
	**/
	...
	{
		$this->debug = $this->fgParams->get('debug'); //FG edit

		if(strpos($this->fgParams->get('strip_list'),'script') === false) $this->removeScripts($this->dom);
		...
		$this->preClean(); //FG edit

		$this->grabImages(); //FG edit

		$this->grabVideos(); //FG edit
		...
		$relaxParsing	= $this->fgParams->get('relax_parsing'); //FG edit
	...
	*/
	function prepArticle($articleContent) {
		...
		if(strpos($this->fgParams->get('strip_list'),'embed') === false) $this->clean($articleContent, 'embed');
		if(strpos($this->fgParams->get('strip_list'),'object') === false) $this->clean($articleContent, 'object');
		if(strpos($this->fgParams->get('strip_list'),'iframe') === false) $this->clean($articleContent, 'iframe');
		...
		***/
		...
		for ($i = $articleParagraphs->length-1; $i >= 0; $i--)
		{
			...
			$embedCount  = strpos($this->fgParams->get('strip_list'),'embed') ? $articleParagraphs->item($i)->getElementsByTagName('embed')->length : 0;
			$objectCount = strpos($this->fgParams->get('strip_list'),'object') ? $articleParagraphs->item($i)->getElementsByTagName('object')->length : 0;
			$iframeCount = strpos($this->fgParams->get('strip_list'),'iframe') ? $articleParagraphs->item($i)->getElementsByTagName('object')->length : 0;
	...
	**/
	...
	/***
	* grabImages - get images from likely containers
	* FG edit
	* @return DOMElement
	**/
	protected function grabImages($page=null) {
		$grabLikelyCandidates = $this->flagIsActive(self::FLAG_GRAB_LIKELY_IMAGES);
		if (!$page) $page = $this->dom;
		$allElements = $page->getElementsByTagName('*');

		$articleImages = $this->dom->createElement('div');
		$articleImages->setAttribute('id', 'readability-images');
	...
	//	$nodesToScore = array();
		...
		//for ($nodeIndex=$targetList->length-1; $nodeIndex >= 0; $nodeIndex--) {
			...
			/* Keep likely candidates */
			if ($grabLikelyCandidates) {
				if(strpos($this->regexps['likelyImageCandidates'],'x/i') AND $likelyCandidates = $this->fgParams->get('likelyCandidates','')) {
					$likelyCandidates = explode(',',$likelyCandidates);
					$this->regexps['likelyImageCandidates'] = str_replace('x/i',implode('|',$likelyCandidates).'/i',$this->regexps['likelyImageCandidates']);
				} else {
					$this->regexps['likelyImageCandidates'] = str_replace('|x/i','/i',$this->regexps['likelyImageCandidates']);
				}
				$likelyMatchString = $node->getAttribute('class') . $node->getAttribute('id');
				if (
					preg_match($this->regexps['likelyImageCandidates'], $likelyMatchString) &&
					$tagName != 'BODY'
				)
				{
					$imageList = $node->getElementsByTagName('img');
					$imageContainer = $this->dom->createElement('div');
					$imageContainer->setAttribute('id', 'readability-'.$likelyMatchString);
					for ($imgIndex = 0; ($imgNode = $imageList->item($imgIndex)); $imgIndex++) {
						$imageContainer->appendChild($imgNode);
					}
					$articleImages->appendChild($imageContainer);
				}
			}
		}
		$this->articleImages = $articleImages;
	//	jexit($articleImages->getElementsByTagName('img')->length.$articleImages->innerHTML);
	}

	/***
	* grabVideos - get videos from likely containers
	* FG edit
	* @return DOMElement
	**/
	protected function grabVideos($page=null) {
		$grabLikelyCandidates = $this->flagIsActive(self::FLAG_GRAB_LIKELY_VIDEOS);
		if (!$page) $page = $this->dom;
		$allElements = $page->getElementsByTagName('*');

		$articleVideos = $this->dom->createElement('div');
		$articleVideos->setAttribute('id', 'readability-videos');
	...
	//	$nodesToScore = array();
		...
		//for ($nodeIndex=$targetList->length-1; $nodeIndex >= 0; $nodeIndex--) {
			...
			if ($grabLikelyCandidates) {
				if(strpos($this->regexps['likelyVideoCandidates'],'x/i') AND $likelyCandidates = $this->fgParams->get('likelyVideoCandidates','')) {
					$likelyCandidates = explode(',',$likelyCandidates);
					$this->regexps['likelyVideoCandidates'] = str_replace('x/i',implode('|',$likelyCandidates).'/i',$this->regexps['likelyVideoCandidates']);
				} else {
					$this->regexps['likelyVideoCandidates'] = str_replace('|x/i','/i',$this->regexps['likelyVideoCandidates']);
				}
				$likelyMatchString = $node->getAttribute('class') . $node->getAttribute('id');
				if (
					preg_match($this->regexps['likelyVideoCandidates'], $likelyMatchString) &&
					$tagName != 'BODY'
				)
				{
					$videoList = array();
					if(strpos($this->fgParams->get('strip_list'),'embed')) $videoList[] = $node->getElementsByTagName('embed');
					if(strpos($this->fgParams->get('strip_list'),'object')) $videoList[] = $node->getElementsByTagName('object');
					if(strpos($this->fgParams->get('strip_list'),'iframe')) $videoList[] = $node->getElementsByTagName('iframe');

					$videoContainer = $this->dom->createElement('div');
					$videoContainer->setAttribute('id', 'readability-'.$likelyMatchString);
					foreach($videoList as $videoNodeList) {
						for ($vidIndex = 0; ($vidNode = $videoNodeList->item($vidIndex)); $vidIndex++) {
							$videoContainer->appendChild($vidNode);
						}
					}
					$articleVideos->appendChild($videoContainer);
				}
			}
		}
		$this->articleVideos = $articleVideos;
	//	jexit($articleVideos->getElementsByTagName('img')->length.$articleVideos->innerHTML);
	}
		...
		for ($i=$curTagsLength-1; $i >= 0; $i--) {
			...
			if ($weight + $contentScore < 0) {
				...
				**/
				...
				$iframe    = $tagsList->item($i)->getElementsByTagName('iframe')->length;
				$object    = $tagsList->item($i)->getElementsByTagName('object')->length;
				...
				if ( $img > $p || $iframe > $p || $object > $p) {
//FG edit				//$toRemove = true; too punitive!!
						//we clean dangerous stuff later, based on parameters
