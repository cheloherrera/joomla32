...
class FeedgatorModelTools extends JModel
{
	...
	function checkLatestVersion(&$fgParams,$version=FG_VERSION,$stable='3190',$dev='5405')
	{
		...
		if($page = FeedgatorUtility::getUrl($url,$fgParams->get('scrape_type'))) {
			$stable = $this->loadJoomlaCode($page);
			$stable['upgrade'] = 0;
		}
		if((!$page OR $fgParams->get('notify_dev')) AND $url2) {
			if($page = FeedgatorUtility::getUrl($url2,$fgParams->get('scrape_type'))) {
	...
	{
		if(J_VERSION >= 1.6) {
			$query = 	'SELECT extension_id FROM #__extensions WHERE type = \'plugin\' AND element = \'feedgator_system\'';
		} else {
			$query = 	'SELECT id FROM #__plugins WHERE element = \'feedgator_system\'';
		}
		$this->_db->setQuery($query);
