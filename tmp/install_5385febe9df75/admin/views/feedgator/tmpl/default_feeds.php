   	...
   	<table class="adminlist" cellspacing="1">
		<thead>
			<tr>
				...
				<th width="1%" nowrap="nowrap"><?php echo JHTML::_('grid.sort',   (J_VERSION < 1.6) ? 'Front Page' : 'Featured', 'front_page', @$this->lists['order_Dir'], @$this->lists['order'] ); ?></th>
       			...
       			<?php if(J_VERSION < 1.6) { ?>
		        	<th width="8%" nowrap="nowrap" class="title"><?php echo JHTML::_('grid.sort',   'Section', 'section_name', @$this->lists['order_Dir'], @$this->lists['order'] ); ?></th>
		        <?php } ?>
			...
			for ($i=0, $n=count( $this->rows ); $i < $n; $i++) {
				...
				<tr class="<?php echo "row$k"; ?>">
					...
					<td width="2%" align="center">
						...
						<img src="<?php echo $admin_img.( $row->front_page ? ((J_VERSION < 1.6) ? 'tick.png' : 'featured.png') : 'disabled.png' );?>" width="16" height="16" border="0" alt="<?php echo ( $row->front_page ) ? JText::_( 'Yes' ) : JText::_( 'No' );?>" /></a>
					</td>
					<td align='left'><?php echo $row->feed; ?></td>
					<td align='left'><?php echo $row->content_type; ?></td>
	       			<?php if(J_VERSION < 1.6) { ?>
						<td align='left'><?php echo $row->section_name; ?></td>
					<?php } ?>
