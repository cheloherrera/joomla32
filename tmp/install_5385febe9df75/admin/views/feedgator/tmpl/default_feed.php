...
<?php } ?>
     ...
     } else if (form.datacatid.value == "-1") {
...
<form action="index.php" method="post" name="adminForm" id="adminForm">
	...
	if(J_VERSION < 1.6) {
		...
		$pdata = $this->fgParams->getParams('data','feed_3');
		echo FeedgatorHelper::renderForm($pdata,true,array('control'=>'data'));
		$pdata = $this->fgParams->getParams('data','feed_1');
		echo FeedgatorHelper::renderForm($pdata,true,array('control'=>'data'));
		$pdata = $this->fgParams->getParams('params','feed_2');
		echo FeedgatorHelper::renderForm($pdata,true);
	...
	if($ext = $this->fgParams->get('content_type')) {
		$model = FGFactory::getPluginModel();
		$plugin = $model->getPlugin($ext);
		$plugin->getParams($this->fgParams->get('id'),true);

		if(J_VERSION < 1.6) {
			$plugparams = $plugin->params->render('pluginparams');
		} else {
			$pdata = $plugin->params->getParams('pluginparams');
			$plugparams = FeedgatorHelper::renderForm($pdata,false,array('fieldset_id'=>'pluginparams'));
		}
	} else {
		$plugparams = '<fieldset class="adminform"><ul class="adminformlist"><li>'.JText::_('FG_PLG_PARAMS_NOT_LOADED').'</li></ul></fieldset>';
	}
