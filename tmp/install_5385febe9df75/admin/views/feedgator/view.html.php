...
class FeedgatorViewFeedgator extends JView
{
	function display($tpl = null)
	{
		...
		if($tpl) {
			...
			if($tpl == 'settings') {
				...
				if(J_VERSION >= 1.6) {
					if($component->params->get('published',null) !== null) {
						$component->params->set('published',null);
					}
				}
				$xmlFile = JPATH_ADMINISTRATOR.DS.'components'.DS.'com_feedgator'.DS.'views'.DS.'feedgator'.DS.'tmpl'.DS.'default_settings.xml';
			...
			elseif($tpl == 'feed' OR $tpl == 'feed_default') {
				$this->buildEditLists($fgParams,$tpl);
				$fgParams->loadSetupFile( JPATH_ADMINISTRATOR.DS.'components'.DS.'com_feedgator'.DS.'views'.DS.'feedgator'.DS.'tmpl'.DS.'default_'.$tpl.'.xml' );
				...
				if($edit AND JRequest::getCmd('task') != 'editdefault') {
					...
					importFunc = function(type) {
						...
						var obj = new Object();
						obj.id = $cid;
						obj.title = $('datatitle').getProperty('value');
						arr[0] = obj;
						setupFeeds(arr,type);
					}
			...
			$fgParams = $model->getParams();
			$version_data = $toolsModel->checkLatestVersion($fgParams);
			$base = (boolean)$fgParams->get('base');
	...
	function buildEditLists(&$fgParams,$tpl)
	{
		$user			= JFactory::getUser();
		$db				= JFactory::getDBO();
		$model 			= FGFactory::getFeedModel();
		...
		$default = false;
		$dynaLists = FeedgatorHelper::getDynaLists($fgParams,$default);
