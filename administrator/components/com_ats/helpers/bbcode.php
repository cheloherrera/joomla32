...
/**
 * A simple BBcode parser library. It supports the following bbCode tags:
 * b, i, u, s, size, color, img, url, list, code, quote
 */
class AtsHelperBbcode
{
	static public function parseBBCode($text, $nestingLevel = 0)
	{
		...
		while($bracketStart !== false) {
			...
			if(in_array($tag,array('SIZE','COLOR','B','I','U','S','LIST','IMG','URL','CODE','QUOTE'))) {
				// Find the end tag
				list($endTagStart, $endTagEnd) = self::findEndTag($tag, $text, $bracketEnd);

				// Exceptions where an end tag is not expected
				$exception = false;
				if($tag == '*') $exception = true;
				if(($tag == 'IMG' ) && is_null($tagData)) $exception = true;
				...
				if(($endTagStart != false) && !$exception && ($nestingLevel <= 15)) {
					...
					switch($tag) {
						case 'SIZE':
							...
							break;
						case 'COLOR':
							if(empty($tagParam)) $tagParam = '';
							$color = strtolower(trim($tagParam));
							$acceptedColors = array('aqua','black','blue','fuchsia','gray','grey','green','lime','maroon','navy','olive','purple','red','silver','teal','white','yellow','beige','brown','chocolate','cyan','darkblue','darkcyan','darkgray','darkgrey','darkorange','darkred','dimgray','dimgrey','gold','goldenrod','greenyellow','indigo','ivory','khaki','lavender','lightblue','lightcyan','lightgray','lightgrey','lightgreen','lightyellow','navy','orangered','pink','plum','skyblue','slateblue','snow','tan','tomato','whitesmoke');
						...
						case 'IMG':
							...
							if(!empty($imgURL)) $imgURL = filter_var($imgURL, FILTER_SANITIZE_URL);
							if(!empty($imgURL)) {
						...
						case 'URL':
							...
							if(!empty($url)) $url = filter_var($url, FILTER_SANITIZE_URL);
							if(!empty($url)) {
								$anchorText = trim($innerText);
								if(empty($anchorText)) $anchorText = $url;
								$anchorText = htmlentities($anchorText, ENT_QUOTES);
								$newText = '<a href="'.htmlentities($url, ENT_COMPAT).'">'.$anchorText.'</a>';
							} else {
								$newText = '';
							}
							$processedTag = true;
	...
	private static function replace($from, $to, $new, $old)
	{
		if($from > $to) {
			$x = $from;
			$from = $to;
	...
	private static function findEndTag($tag, $text, $offset = 0)
	{
		...
		while($bracketStart !== false) {
			...
			if($found) {
				$ret = array($bracketStart, $bracketEnd);
				$bracketStart = false;
			} else {
